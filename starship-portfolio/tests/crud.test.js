...
import { normalizePortfolio, saveEduMaxim, saveProject, saveMissionUpdate } from '../src/portfolioData.js';
...
test('CRUD persists, preserves other sections, rejects stale and unauthorized writes', async () => {
  ...
  next = saveEduMaxim(saved.body, { ...saved.body.edumaxim, title: 'Updated Edumaxim', features: ['One'] });
  saved = await call(dataHandler, 'PUT', next, cookie);
  assert.equal(saved.body.edumaxim.title, 'Updated Edumaxim');
  assert.deepEqual(saved.body.edumaxim.features, ['One']);
